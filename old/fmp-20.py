...
class PennyStockVolumeScanner:
    ...
    def scan_for_volume_spikes(self, max_market_cap=1000000000, spike_multiplier=2.0, max_stocks=50, 
                            volume_spike_threshold=1.5, min_absolute_volume=1000):
        ...
        # Consumer Cyclical, Energy, Technology, Industrials, Financial Services, Basic Materials, 
        # Communication Services, Consumer Defensive, Healthcare, Real Estate, Utilities, Industrial Goods, 
        # Financial, Services, Conglomerates
        filter_out_sectors = ['Financial Services', 'Financial']
        stocks = self.filter_out_sectors(stocks, filter_out_sectors)
        
        for stock in stocks:
            print(f"Checking stock: {stock['symbol']} - Volume: {stock.get('volume', 0):,}")
    ...
    def filter_out_sectors(self, stocks, unwanted_sectors):
        """
        Filter out stocks from unwanted sectors
        
        Args:
            stocks: List of stock dictionaries from get_low_market_cap_stocks
            unwanted_sectors: List of sector names to exclude (case-insensitive)
        
        Returns:
            Filtered list of stocks excluding the unwanted industries
        """
        if not unwanted_sectors:
            return stocks
            
        # Convert unwanted industries to lowercase for case-insensitive comparison
        unwanted_sectors_lower = [sector.lower() for sector in unwanted_sectors]
        
        filtered_stocks = []
        excluded_count = 0
        
        for stock in stocks:
            stock_sector = stock.get('sector', '') or ''  # Handle None values
            stock_sector_lower = stock_sector.lower()
            
            if stock_sector_lower not in unwanted_sectors_lower:
                filtered_stocks.append(stock)
            else:
                excluded_count += 1
        
        print(f"🚫 Filtered out {excluded_count} stocks from unwanted industries: {', '.join(unwanted_sectors)}")
        print(f"✅ Remaining stocks: {len(filtered_stocks)}")
    ...
    def get_low_market_cap_stocks(self, max_market_cap=1000000000, min_volume=50000):
        ...
        print("-> get_low_market_cap_stocks")
        url = f"{self.base_url}/stock-screener"
        params = {
            'marketCapLowerThan': max_market_cap,
            'volumeMoreThan': min_volume,
            'priceMoreThan': 20,  # Penny stocks above $0.01
            'priceLowerThan': 30,   # Under $10
            'limit': 1000, # TODO change to 1000
            'apikey': self.api_key
        }
        ...
        try:
            ...
            print("get_low_market_cap_stocks ->")
            # next pretty print the response for debugging
    ...
    def get_average_volume(self, symbol, days=30):
        ...
        params = {
            'timeseries': days,
            'apikey': self.api_key  # Removed 'serietype': 'line' to include volume
        }

        try:
            # print(f"📡 Requesting average volume for {symbol}: {url} with params {params}")
            response = requests.get(url, params=params)
            response.raise_for_status()
            data = response.json()
            ...
            print(f"Found {len(volumes)} days of volume data for {days}")
            if not volumes:
                print("\n\n\n*******************************")
                print(f"⚠️ No valid volume entries for {symbol}")
                print("******************************* \n\n\n")
                return None

            if len(volumes) < days:
                print("\n\n\n *******************************")
                print(f"⚠️ Only {len(volumes)} volume days available for {symbol} (expected {days})")
                print("*******************************\n\n\n")
    ...
    def get_historical_volume_data(self, symbol, days=90):
        ...
        print("-> get_historical_volume_data")
        ...
        try:
            print(f"📡 Requesting historical volume data for {symbol}: {url} with params {params}")
            response = requests.get(url, params=params)
            response.raise_for_status()
            data = response.json()
            historical = data.get('historical', [])
            ...
            if not historical:
                print(f"⚠️ No historical volume data returned for {symbol}")
                return []
            print("get_historical_volume_data ->")
            return historical
    ...
    def analyze_volume_spike(self, historical_data, spike_multiplier=2.0, recent_days=30):
        ...
        # TODO understand method better
        print("-> analyze_volume_spike")
...
def main():
    ...
    results = scanner.scan_for_volume_spikes(
        ...
        max_stocks=250,                 # Analyze top 20 most active stocks
