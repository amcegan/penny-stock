...
class PennyStockVolumeScanner:
    ...
    def get_average_volume(self, symbol, days=30):
        ...
        try:
            ...
            print(f"******** Fetched {len(historical)} days of historical data for {symbol}")
            if not historical:
            ...
            if len(volumes) < days - 5: # Allow some leeway for missing data
                print("\n\n\n *******************************")
    ...
    def analyze_volume_spike(self, historical_data, spike_multiplier=2.0, recent_days=5):
        ...
        if len(spike_days) > 0:
            ...
            # Find the most recent spike date
            most_recent_spike_date = spike_days['date'].max()
            
            # Calculate days since the most recent spike
            days_since_recent_spike = (datetime.now().date() - most_recent_spike_date.date()).days
            
            return {
                'has_spike': True,
                'max_spike_ratio': max_spike,
                'max_spike_date': max_spike_date.strftime('%Y-%m-%d'),
                'most_recent_spike_date': most_recent_spike_date.strftime('%Y-%m-%d'),
                'days_since_recent_spike': days_since_recent_spike,
...
def main():
    ...
    results = scanner.scan_for_volume_spikes(
        max_market_cap=100000000000,     # $1B max market cap
        spike_multiplier=2.0,          # 2x historical volume spike threshold
        max_stocks=40,                 # Analyze top 20 most active stocks
